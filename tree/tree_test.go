...
func TestMapTree(t *testing.T) {
	...
	v := tree1.Get(style.UnixChain("animals.0.species_name"))
	if s, ok := v.(string); !ok {
	...
	tree1.Set(style.UnixChain("name"), "Sea World")
	tree1.Set(style.UnixChain("animals.0.species_name"), "Dolphin")
	tree1.Set(style.UnixChain("keepers.3"), map[string]interface{}{"Name": "Mike", "Age": 29})
	tree1.Set(style.UnixChain("animals.4.1.3"), "a mistake")

	if tree1.Get(style.UnixChain("name")) != "Sea World" {
		t.Fatalf("expected zoo name `Sea World`, but got %s", tree1.Get(style.UnixChain("sea")))
	}
	if tree1.Get(style.UnixChain("animals.0.species_name")) != "Dolphin" {
		t.Fatalf("expected species name `Dolphin`, but got %s", tree1.Get(style.UnixChain("animals.0.species_name")))
	}
	if tree1.Get(style.UnixChain("keepers.3.name")) != "Mike" {
		t.Fatalf("expected keeper name `Mike`, but got %s", tree1.Get(style.UnixChain("keepers.3.name")))
	}

	tree1.Remove(style.UnixChain("animals.4.1.3"))
	tree1.Remove(style.UnixChain("Keepers.1.Name"))
	tree1.Remove(style.UnixChain("Keepers.1.Age"))
	...
	mapTree2.Remove(style.UnixChain("Keepers.3.Name"))
	mapTree2.Remove(style.UnixChain("Keepers.3.Age"))
	mapTree2.Set(style.UnixChain("Name"), "Universe World")
	mapTree2.Set(style.UnixChain("Keepers.0.Age"), 100)
